 ...
 *
 * Thirty-seven categories and twenty-eight SKUs is a megabyte of JSON — less than one of its own
 * product photographs — so the snapshot ships in the repo and the database becomes an optimisation
 * rather than a dependency. Every loader tries the live path first and falls back here, so a clone
 * renders the whole store with `pnpm dev` and nothing else running, Vercel serves it with no
 * environment variables, and a database outage degrades to yesterday's catalogue instead of an
 * error page.
 *
 * Generated by scripts/export-catalogue.mts by calling the real loaders, so these objects are
 * shape-identical to the live ones by construction rather than by a second mapping that can drift.
 */
